...
public class InvestWechatDao extends InvestDao {
	...
	public PageBean<InvestBidBean> pageOfInvestBids(int currPage, int pageSize) {
		String sql = " SELECT tb.reward_rate AS rewardRate,  tb.id AS id ,tb.title AS title, tb.repayment_type AS repayment_type, tb.apr AS apr ,tb.period  AS period , tb.period_unit AS periodUnit , tb.amount AS amount, tb.product_id AS product_id, tb.has_invested_amount AS hasInvestedAmount, tb.status AS status ,tb.loan_schedule AS loanSchedule, tb.pre_release_time AS preRelease, tp.type AS type from t_bid tb, t_product tp WHERE tb.product_id = tp.id and status IN(:statusList) ORDER BY CASE WHEN status > 1 THEN 2 ELSE 1 END, pre_release_time DESC ";
	...
	public BidLoanUserBean findInvestBidsUserInfo(long bidId) {
		...
		String sql = "SELECT b.title AS title,b.product_id AS product_id, b.id AS bid_id, tgm.name AS guarantee_mode, u.prov_id AS prov_id,b.apr AS apr,b.period AS period,b.invest_period AS invest_period,b.repayment_type AS repaymentType, b.repayment_source AS repayment_source,b.guarantee_measures AS guarantee_measures,b.guarantee_mode_id AS guarantee_mode_id,u.work_unit AS work_unit,u.start_time AS start_time,u.registered_fund AS registered_fund, u.reality_name AS realityName,u.id_number AS idNumber,u.credit_level_id AS creditLevelId,u.sex AS sex ,u.education AS education, u.marital AS marital  ,u.annual_income AS annualIncome ,u.net_asset AS netAsset,u.work_experience AS workExperience ,u.house AS house ,u.car AS car ,b.preaudit_suggest AS preauditSuggest ,b.description AS description  FROM  t_bid b LEFT JOIN t_user_info u  ON b.user_id=u.user_id LEFT JOIN t_guarantee_mode tgm ON b.guarantee_mode_id=tgm.id WHERE b.id=:bidId";
