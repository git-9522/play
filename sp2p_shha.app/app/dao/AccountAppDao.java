...
public class AccountAppDao extends BaseDao<t_user> {
	 ...
	 * @createDate 2016-4-6
	 */
	public PageBean<UserScoreRecordApp> pageOfUserScoreRecord (int currPage, int pageSize, long userId) {
		StringBuffer querySQL = new StringBuffer("SELECT su.id AS id, u.name AS userName, su.deal_type AS dealType, su.time AS time, su.score AS score, su.summary AS summary FROM t_score_user su LEFT JOIN t_user u ON su.user_id = u.id WHERE 1 = 1 ");
		...
		querySQL.append(" AND u.id = :userId ");
		countSQL.append(" AND u.id = :userId ");
		condition.put("userId", userId);
		
		querySQL.append(" order by id desc ");
		
		return pageOfBeanBySQL(currPage, pageSize, countSQL.toString(), querySQL.toString(), UserScoreRecordApp.class, condition);
	}
}